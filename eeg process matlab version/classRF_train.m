...
%      control the RF
%  £¨1£© extra_options.replace = 0 or 1 (default is 1) sampling with or without
%                           replacement
%  £¨2£© extra_options.classwt = priors of classes. Here the function first gets
...
%  £¨3£© extra_options.cutoff (Classification only) = A vector of length equal to number of classes. The ?winning?
...
%                       vote wins). 
%  £¨4£© extra_options.strata = (not yet stable in code) variable that is used for stratified
%                       sampling. I don't yet know how this works. Disabled
%                       by default
%  £¨5£© extra_options.sampsize =  Size(s) of sample to draw. For classification, 
...
%  £¨6£© extra_options.nodesize = Minimum size of terminal nodes. Setting this number larger causes smaller trees
%                   to be grown (and thus take less time). Note that the default values are different
%                   for classification (1) and regression (5).
%  £¨7£© extra_options.importance =  Should importance of predictors be assessed?
%  £¨8£© extra_options.localImp = Should casewise importance measure be computed? (Setting this to TRUE will
%                   override importance.)
%  £¨9£© extra_options.proximity = Should proximity measure among the rows be calculated?
%  £¨10£©extra_options.oob_prox = Should proximity be calculated only on 'out-of-bag' data?
%  £¨11£©extra_options.do_trace = If set to TRUE, give a more verbose output as randomForest is run. If set to
%                   some integer, then running output is printed for every
%                   do_trace trees.
%  £¨12£©extra_options.keep_inbag Should an n by ntree matrix be returned that keeps track of which samples are
